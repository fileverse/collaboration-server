...
function fakeSocket(role: "owner" | "editor"): AppSocket {
  return { id: "s1", data: { authenticated: true, documentId: "doc-1", sessionDid: "room-did", role, appType: "ddoc" } } as unknown as AppSocket;
}
...
describe("handleSetDocumentMeta", () => {
  ...
  it("rejects a non-owner", async () => {
    const cb = vi.fn();
    await handleSetDocumentMeta(deps, fakeSocket("editor"), { editLock: "el", title: "t" }, cb);
